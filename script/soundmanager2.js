...
function SoundManager(smURL, smID) {
  ...
  this.requiredFormats = {
    ...
    'audio/mpeg': true, // MP3 format support. Safari needs "audio/mpeg"
...
if (typeof window.console !== 'undefined' && console.log) {
          console.log('Audio()::canplaythrough(), type: '+sType);
}
          handler(true, e);
          a = null;
        }, false);
...
// DEV - remove later
if (typeof window.console !== 'undefined' && console.log) {
          console.log('Audio()::error(), code: '+this.error.code+', type: '+sType);
}
  ...
  _initMovie = function() {
    ...
    if (_s.o) {
      _s._wD('soundManager::initMovie(): Got '+_s.o.nodeName+' element ('+(_didAppend?'created via JS':'static HTML')+')',1);
      _wDS('waitEI');
    }
  ...
  _beginInit = function() {
    ...
    _createMovie(); // ensure creation if not already done
    _initMovie();
    _initPending = true;
    return true;
  };
  ...
  _onTimer = function() {
    var i, timerCount = 0, oS = null;
    for (i=_s.soundIDs.length; i--;) {
      oS = _s.sounds[_s.soundIDs[i]];
...
return true;
    ...
    this.togglePause = function() {
      ...
      if (_t.playState === 0) {
        _t.play({
          position: (_s.flashVersion === 9 && !_t.useHTML5 ? _t.position:_t.position / 1000)
        });
        return false;
      }
      if (_t.paused) {
...
*/
    ...
    this._whileplaying = function(nPosition, oPeakData, oWaveformDataLeft, oWaveformDataRight, oEQData) {
      ...
      if (_s.flashVersion > 8 && !_t.useHTML5) {
